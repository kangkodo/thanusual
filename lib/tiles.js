...
// CARTO basemap styles the proxy will serve. The client picks by prefers-color-scheme.
export const TILE_STYLES = { light: "light_all", dark: "dark_all" };

export function parseTilePath(raw) {
  const s = String(raw || "").replace(/^\/+/, "");
  const m = /^(?:(light|dark)\/)?(0|[1-9]\d*)\/(0|[1-9]\d*)\/(0|[1-9]\d*)(@2x)?\.png$/.exec(s);
  if (!m) return null;
  const z = Number(m[2]);
  const x = Number(m[3]);
  const y = Number(m[4]);
  ...
  return { z, x, y, retina: Boolean(m[5]), style: m[1] || "light" };
}
...
export const TILE_CACHE_VERSION = "v2";
...
export function cacheKeyUrl(requestUrl, tile) {
  ...
  return `${origin}/tiles/${TILE_CACHE_VERSION}/${tile.style || "light"}/${tile.z}/${tile.x}/${tile.y}${r}.png`;
}

export function upstreamUrl(z, x, y, retina, key, style = "light") {
  const r = retina ? "@2x" : "";
  const set = Object.hasOwn(TILE_STYLES, style) ? TILE_STYLES[style] : TILE_STYLES.light;
  return `https://a.basemaps.cartocdn.com/${set}/${z}/${x}/${y}${r}.png?key=${encodeURIComponent(key)}`;
}
