...
test("cache key is versioned, canonical across aliases, and drops the query", () => {
  const tile = { z: 11, x: 1748, y: 796, retina: false, style: "light" };
  const want = "https://thanusual.pages.dev/tiles/v2/light/11/1748/796.png";
  ...
  assert.equal(cacheKeyUrl(PROD, { ...tile, style: "dark" }), want.replace("/light/", "/dark/"));
  assert.doesNotThrow(() => new Request(cacheKeyUrl(PROD, tile)));
});

test("parses png, @2x, and the optional light/dark style", () => {
  assert.deepEqual(parseTilePath("11/1748/796.png"), { z: 11, x: 1748, y: 796, retina: false, style: "light" });
  assert.deepEqual(parseTilePath("/11/1748/796@2x.png"), { z: 11, x: 1748, y: 796, retina: true, style: "light" });
  assert.deepEqual(parseTilePath("dark/11/1748/796@2x.png"), { z: 11, x: 1748, y: 796, retina: true, style: "dark" });
  assert.deepEqual(parseTilePath("light/0/0/0.png"), { z: 0, x: 0, y: 0, retina: false, style: "light" });
  assert.deepEqual(parseTilePath("18/262143/262143.png"), { z: 18, x: 262143, y: 262143, retina: false, style: "light" });
});
...
test("rejects bad tile paths", () => {
  ...
  assert.equal(parseTilePath("night/11/0/0.png"), null);
  assert.equal(parseTilePath("DARK/11/0/0.png"), null);
  assert.equal(parseTilePath("dark/dark/11/0/0.png"), null);
...
test("upstream url puts the key in the query string and honors @2x", () => {
  ...
  assert.equal(upstreamUrl(11, 1, 2, false, "k", "dark").startsWith("https://a.basemaps.cartocdn.com/dark_all/11/1/2.png?key="), true);
  assert.equal(upstreamUrl(11, 1, 2, false, "k", "sepia").startsWith("https://a.basemaps.cartocdn.com/light_all/"), true);
  assert.equal(upstreamUrl(11, 1, 2, false, "k", "constructor").startsWith("https://a.basemaps.cartocdn.com/light_all/"), true);
});
